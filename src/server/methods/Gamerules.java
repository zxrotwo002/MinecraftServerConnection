...
import server.schemas.TypedGameRule;
import server.schemas.UntypedGameRule;
...
public class Gamerules implements MessageHandler {
    private final String method = "minecraft:gamerules";
    //id-range 3000-3999
    private final int idBase = 3000;
    ...
    //3010
    private TypedGameRule[] gameRules;

    Server server;
    public Gamerules(Server server) {
        this.server = server;
    }

    //3010
    public TypedGameRule[] getGameRules() {
        int id = 10;
        ...
        try {
            ...
            TypedGameRule[] gameRules1 = new TypedGameRule[response.get("result").getAsJsonArray().size()];
            AtomicInteger i = new AtomicInteger(0);
            response.get("result").getAsJsonArray().forEach(jsonElement ->
                    gameRules1[i.getAndIncrement()] = server.gson.fromJson(jsonElement, TypedGameRule.class));
            return gameRules1;
    ...
    //3020
    public boolean updateGameRule(UntypedGameRule untypedGameRule) {
        JsonObject params = new JsonObject();

        int id = 20;
        String method = getSubmethod("update");
        params.add("gamerule", server.gson.toJsonTree(untypedGameRule));
        ...
        try {
            JsonObject response = future.get(timeout, TimeUnit.SECONDS);
            pendingRequests.remove(id);
            TypedGameRule typedGameRule = server.gson.fromJson(response.get("result").getAsJsonObject(), TypedGameRule.class);
            return typedGameRule.key.equals(untypedGameRule.key) && typedGameRule.value.equals(untypedGameRule.value);
        } catch (InterruptedException | ExecutionException e) {
            System.err.println("Error while waiting for response: " + e.getMessage());
        } catch (TimeoutException e) {
            System.err.println("Timed out waiting for response with ID: " + (idBase + id));
